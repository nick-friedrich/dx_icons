...
fn main() {
    ...
    let dest_path = Path::new(&out_dir).join("lucide_icons.rs");

    let mut code = String::new();

    // ---------- LucideIcon enum ----------
    code.push_str("/// All available Lucide icons.\n");
    code.push_str("#[derive(Clone, Copy, PartialEq, Eq, Hash, Debug)]\n");
    code.push_str("pub enum LucideIcon {\n");
    for name in icons.keys() {
        let variant = to_pascal_case(name);
        code.push_str(&format!("    /// Lucide icon: `{name}`\n"));
        code.push_str(&format!("    {variant},\n"));
    }
    code.push_str("}\n\n");

    // ---------- LucideIcon::svg_content ----------
    code.push_str("impl LucideIcon {\n");
    ...
    for name in icons.keys() {
        ...
        code.push_str(&format!("/// Lucide icon: `{name}`\n"));
        code.push_str(&format!(
            "/// Shorthand for `Icon {{ icon: LucideIcon::{variant} }}`\n"
        ));
        ...
        code.push_str("    rsx! {\n");
        code.push_str(&format!(
            "        Icon {{ icon: LucideIcon::{variant}, size, color, stroke_width, class, fill }}\n"
        ));
        code.push_str("    }\n");
        code.push_str("}\n\n");
    }
